...
Existe el enlace duro (Hard link): Donde se referencia un nombre a un cierto Inodo, y ciertos datos.
Y un enlace blando (soft link): Donde se crea un acceso directo de referencia a ese inodo, apunta a ese inodo, cuando el inodo desaparece, deja de apuntar a el.
- El enlace duro permite adquirir el mismo Inodo que el archivo, e aumenta el contador de links, permitiendo que existan mas rutas para acceder a los datos y estos tengan mayor persistencia.
- Si se borra el archivo original, pero los datos estan referenciados en otro link, entonces estos datos no se borran.
- En cambio el enlace blando es un enlace externo, que no aumenta el contador de links.

**Comandos:**
stat archivo: Ver estadisticas completas (detalladas) del archivo.
ln Archivo nuevoArchivo: Crear enlace duro a un archivo.
ln -s Archivo  nuevoArchivo: Crea un enlace simbolico con el parametro -s.
...
- **chown:** Cambia el propietario de varios archivos y directorios.
- **chgrp:** Cambia el grupo de varios archivos y directorios.
**EJEMPLOS DE USO PRACTICO:**
chwon pedro archivo
	chown :usuarios1 archivo
	chown -R mi:usuarios /home/me/proceso
- chgrp: funciona de forma equivalente. Sin embargo chown es especificamente para el usuario, y chgrp es para los grupos.

**PERMISOS DE LOS DIRECTORIOS:**
Los directorios tienen una responsabilidad mayor a los archivos. Un cierto usuario con permisos puede borrar un archivo aun si no tiene permisos de nada (000).
- Esto sucede debido a que los directorios al asignarle permisos a ciertos usuarios, estos tienen poder de desenlazar todos los nombres que aparecen dentro de ese directorio. Por lo que los permisos sobre directorio son mas fundamentales que los permisos sobre los archivos.s
**Read:** Leer directorio. Sabes que existe, y que tiene: "ls"
**Write:** Escribir en el directorio (permite modificar y borrar).
**Execute:** Permite ejecutar la carpeta, donde podrias navegar con "cd".

MIentras que tenga el execute (X) activado, el directorio es capaz de ejecutar cualquier archivo interno, aun si no tiene permisos particulares.