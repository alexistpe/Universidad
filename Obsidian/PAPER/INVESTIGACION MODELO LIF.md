...
- **¿Cuál es el porcentaje de datos faltantes en mi dataset y cuál es su patrón?**
    ...
    Para identificar los datos faltantes en el dataset actual, necesito primero crear un script automatico de recoleccion de estos datos, utilizando la API de meteostat.
    Posteriormente crear un segundo script que permita analizar los datos recuperados e identificar la cantidad de datos faltantes en porcentaje.
    Una vez hecho esa recoleccion, podemos pasar a analizar y decidir que hacer con los datos faltantes y cual es su gravedad.
    
	- #### **Scripts**
    
	    Estan realizados en Python 3.
	    
	    - **Script 1: extraer_datos.py**
	        ...
	        - **Segunda version:** Extrae los datos directamente de los archivos ISD proporcionados por el NOAA.
	            ...
	            1. El script procesará **todos** los `.txt` de la carpeta, fusionará los datos por estación (código ICAO) y creará un archivo de salida por cada estación encontrada.
	    ...
	    - **Script 2:** **analizar_datos.py**
	        - Busca .txt en su propia carpeta
	        - Si hay varios, permite elegir
	        - Analiza faltantes por variable y total
	        - Muestra porcentajes en terminal
	    - **Flujo de uso:**
	        1. python3 ExtraerDatos/extraer_datos.py → genera el .txt
	        2. Copiar el .txt a AnalizarDatos/
	        3. python3 AnalizarDatos/analizar_datos.py → analiza
    
    ### Datos faltantes
    
    Se analizan que faltan la gran mayoria de datos de la precipitacion.
    - Analizar rangos mas ampleos y determinar otras variables faltantes.
    
    Se debe encontrar un metodo para remplazar esa variable, y poder utilizar una metrica de exito.
    - Las opciones son: Utilizar otro atributo, utilizar otro proveedor.
    
    #### Alternativas
	- **ISD:** Extraer los datos directamente del ISD, obteniendo las variable de precipitacion acumulada cada 6 horas.
		- Pregutnas: 
			- ¿Ese dato es suficiente para deteminar si llovio o no?
			- ¿Afecta al entrenamiento del modelo?
	- **GHCN:** Investigar e intentar extraer los datos directamente de aqui.
	- **INTA/SMN:** Psroveedor alternativo que nos brinde un comunicado si llovio o no en esa hora. (precipitacion o aviso).
    ...
    2. **Eliminación de períodos**: Si la laguna es extensa, lo más riguroso es excluir ese período del análisis, documentando la decisión.
    3. **Múltiples estaciones**: Si tienes datos de una estación cercana, puedes usarlos para estimar los valores faltantes mediante correlación, siempre documentando el procedimiento.
    4. **Búsqueda de fuentes alternativas**: Explora el uso de datos de re-análisis (como ERA5) que, aunque no son observaciones puras, están diseñados para ser consistentes en el tiempo y el espacio. Son más apropiados que un pronóstico operativo como MOSMIX.
...
**Tarea:** Escribir un borrador de los 5 pasos anteriores, con el nivel de detalle que permita empezar a programar.
...
- Limitaciones de la adquisicion de datos:
    ...
    4. Fuente más abierta y verificable → SMN WIS2
