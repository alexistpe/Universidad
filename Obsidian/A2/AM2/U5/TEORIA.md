...
## Objetivos actuales:
Identificar que falta expandir o estudiar sobre este tema.
#### Conceptos a revisar:
- Homgenias y nmo homogenias, ¿terminos?
- ¿Cual es la variable dependiente y la dependiente? ¿que sucede con las ecuaciones diferenciales parciales? ¿se derivan 2 variables dependientes o independinetes?
	- "y" es la variable dependiente en las EDO de grado uno. Siendo x comunmente la independiente.
	- Se deriva "y" con respecto a "x", se **abordan ambas variables**.
	- En el caso de variar el orden, este **depende de la mayor derivada**, sin importar la variable.
	- Esto ultimo se sotiene al ver que las **variables independientes NO PUEDEN tener derivadas.**
		- Si ambas son derivadas, entonces existe una tercera variable no escrita aparte de las que esta, y esa es la independiente.
		- ' = Derivado con respecto a la variable independiente del problema
	- Si se quiere derivar la independiente, la puede terminar descartando de la ecuacion.

#### Temas no expandidos:
- **Solucion particular:** Consiste en evaluar la solucion general en un cierto valor del parametro.
- **Polinomio de segundo grado:** Las EDO segundo grado homogenias se resuelven con ese polinomio, y divide a las raices de este en 3 casos.
	- Para la resolucion de EDO de variables no separables o mayores al primer orden, entonces se utiliza el operador diferencial.
	- Si es segundo orden homogenia se utiliza el polinomio de segundo grado.
	- El unico caso donde utilizamos las **integrales** son en las variables separables que **solo suceden en primer orden.**
- **Linealidad:** Se determina que es lineal, cuadratica, etc... segun el exponente de las variables.
- **Notacion de leibniz:** dependiente/independiente.